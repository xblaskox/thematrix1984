...
CYCLES = 30.0
NODES = {
    'The Train Station': 0.9,
    'The Construct':     1.0,
    'Simulatte':1.2,
    'The Oracles Apartement':        1.1,
    'The Merovingians Chateau':       1.3
}
WAREZ = {
    'RootKit':   (15000, 28000),
    'BlackICE':  (2000,  10000),
...
def load_game():
    if os.path.exists(SAVEFILE):
        try:
            ...
            print("Save file is corrupted or empty. Starting fresh.")
            reset_full()
    else:
        reset_full()
    return update_perks()
...
def draw_screen(stdscr, prices):
    ...
    h, w = stdscr.getmaxyx()
    # Header
    hdr = f" The MATRIX 1984 | Cyc {state['cycle']:.1f}/{CYCLES} | Life {state['life']}% | Armor {state['armor']}% | {state['location']} "
    ...
    # Market
    mx = w//3
    stdscr.addstr(2, mx, 'BLACK MARKET')
...
def handle_upload(stdscr, prices):
    ...
    y0 = 5
    stdscr.clear()
    stdscr.addstr(y0, 2, 'Upload: Choose item by number')
    idx = 1
    items = [w for w,cnt in state['inventory'].items() if cnt > 0]
    for w in items:
        cnt = state['inventory'][w]
        pr = prices[w]
        stdscr.addstr(y0+idx, 4, f"{idx}. {w} - {pr} cr (You have: {cnt})")
        idx += 1
    stdscr.addstr(y0+idx, 4, 'Q) Quit')
    stdscr.refresh()
    sel = stdscr.getstr(y0+idx+1, 4).decode().strip()
    ...
    try:
        qty = max(1, min(maxq, int(q)))
        state['credits'] += prices[key] * qty
        state['inventory'][key] -= qty
        state['cycle'] += 0.25
        stdscr.addstr(y0+idx+4, 4, 'Upload complete. Press any key...')
        stdscr.refresh()
        stdscr.getkey()
    except ValueError:
        return
    ...
    while True:
        ...
        if ch == 'R':
            ...
            if random.random() < 0.35:
                ...
                stdscr.addstr(5,2,'Escape failed!')
                stdscr.addstr(7,2,'Press any key...')
                stdscr.refresh(); stdscr.getkey()
                # failed escape, enemy turn follows
        ...
        if hp <= 0:
            ...
            if smith:
                ...
                state['credits'] += gain
                state['inventory']['RootKit'] = state['inventory'].get('RootKit',0) + 1
                stdscr.addstr(10+len(opts),2, f'Smith defeated! +{gain}cr +1 RootKit')
            else:
        ...
        if random.random() > dodge:
            ...
            if state['armor'] > 0:
                absorbed = min(state['armor'], dmg)
                state['armor'] -= absorbed
                dmg -= absorbed
            # Apply remaining to life
            if dmg > 0:
                state['life'] = clamp(state['life'] - dmg, 0, 100)
...
def curses_sim(stdscr):
    ...
    stdscr.addstr(10,5,'[N]ew Game+    [Q]uit')
    stdscr.refresh()
    ch = stdscr.getkey().upper()
    if ch == 'N':
        # Start New Game+ preserving inventory and perks
        state['cycle'] = 1.0
...
Your mission: infiltrate location nodes, trade digital contraband,
evade Agents, and amass credits to arm yourself, free minds and become an Elite Operator.

Agent Smith is out there. Right now. Tracing you. Every move you make within the Matrix sends a ripple through the code. Agents will soon follow. You must survive. Free as many willing minds as you can.
...
def show_acknowledgments():
    print("""
Acknowledgments:
- My family for supporting me in my journey into Tech. My wife Jenni, son Odin and mother Lynn.
- Yellow Tail Tech for the education. My instructors  Alfred, Anthony, Riky and Jerry.
- Lane from Boot.dev for the Python education and help working out bugs.
- Sophos Cohort for going on this journey with me.
- John E. Dell for pioneering the text based BASIC games of the 80s.
- Beta Testers for making this project actually work.
- The creators of the Matrix, which this game is based in.
- Easter eggs in the game include things from: Cyberpunk, Blade Runner, Gundam Wing, Neuromancer, Ghost in the Shell and all things high tech/low life related.
- Special thanks to Kevin Mitnick (R.I.P.), Darknet, 2600 and the irc warez and hacking scene from the 90s. Echo = True. Still.
-
""")
