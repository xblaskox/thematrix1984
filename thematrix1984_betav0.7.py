...
def handle_buy_armory(stdscr):
    ...
    for wp, inf in WEAPONS.items():
        stdscr.addstr(y0+idx, 2, f"{idx}. {wp} {inf['price']}cr")
        idx += 1
    for u, pr in USEABLES.items():
        stdscr.addstr(y0+idx, 2, f"{idx}. {u} {pr}cr")
        idx += 1
    for a, pr in AMMO_PRICES.items():
        stdscr.addstr(y0+idx, 2, f"{idx}. {a} {pr}cr")
        idx += 1
    stdscr.addstr(y0+idx, 2, 'Q) Quit')
def handle_equip(stdscr):
    curses.echo()
    stdscr.addstr(12,2,'Equip which? ')
    key = stdscr.getstr().decode().strip()
    curses.noecho()
    if key == 'Health Pack' and state['useables'][key] > 0:
        state['useables'][key] -= 1
        state['life'] = clamp(state['life'] + 15, 0, 100)
    elif key == 'Armor Kit' and state['useables'][key] > 0:
        state['useables'][key] -= 1
