...
# === Game Data ===
CYCLES = 30.0
NODES = {'Trainstation':0.9, 'Construct':1.0, 'Nebuchadnezzar':1.2, 'Oracle':1.1, 'Chateau':1.3}
WAREZ = {'RootKit':(15000,28000), 'BlackICE':(2000,10000), 'Datashard':(300,1000), 'Trojan':(1000,4200), 'Worm':(18,75)}
WEAPONS = {
...
AMMO_PRICES = {'Beretta Ammo':5, 'MP5K Ammo':3, 'M4 Ammo':7, 'Health Pack':50, 'Armor Kit':100}
...
def update_perks():
    ...
    if all(x in p for x in ['Heavily Armed','Edge Runner','Data Broker']) and 'Elite Operator' not in p:
...
def draw_screen(stdscr, prices):
    ...
    # Health/Armor
    ha = ay + len(state['ammo']) + 1
    stdscr.addstr(ha, ax, 'HEALTH/ARMOR')
    stdscr.addstr(ha+1, ax+2, f"HPacks:{state['ammo']['Health Pack']}")
    stdscr.addstr(ha+2, ax+2, f"AKits: {state['ammo']['Armor Kit']}")
    # Perks
    px = 2*w//3
    stdscr.addstr(2,px, 'PERKS')
    for idx,perk in enumerate(state['perks'], start=3):
        stdscr.addstr(idx, px+2, perk)
    # Market
    mx = px
    stdscr.addstr(ha+4, mx, 'MARKET')
    for idx,(wz,pr) in enumerate(prices.items(), start=ha+5):
        stdscr.addstr(idx, mx+2, f"{wz[:10]:<10}{pr:>6}cr")
    # Footer
...
def handle_download(stdscr, prices):
    curses.echo()
    stdscr.addstr(12,2, 'DOWNLOAD which? ')
    item = stdscr.getstr(12,18,15).decode().title()
    curses.noecho()
    if item not in WAREZ:
        return
    price = prices[item]
    maxq = state['credits'] // price
    curses.echo()
    stdscr.addstr(13,2, f'Qty? (1-{maxq}): ')
    qty_str = stdscr.getstr(13,18,5).decode()
    curses.noecho()
    try:
        qty = max(1, min(maxq, int(qty_str)))
...
def handle_upload(stdscr, prices):
    curses.echo()
    stdscr.addstr(12,2, 'UPLOAD which? ')
    item = stdscr.getstr(12,18,15).decode().title()
    curses.noecho()
...
def handle_buy_armory(stdscr):
    curses.echo()
    y0 = 12
    stdscr.addstr(y0,2, 'ARMORY: Choose #')
    ...
    stdscr.addstr(y0+off+len(AMMO_PRICES)+1,2,'# or Q: ')
    sel = stdscr.getstr().decode()
    curses.noecho()
    if sel.upper() == 'Q':
        return
    try:
        ...
        maxq = state['credits'] // price
        curses.echo()
        stdscr.addstr(y0+off+len(AMMO_PRICES)+3,2, f'Qty? (1-{maxq}): ')
        qty_str = stdscr.getstr().decode()
        curses.noecho()
        qty = max(1, min(maxq, int(qty_str)))
        state['credits'] -= price * qty
        inv[key] = inv.get(key, 0) + qty
        if ak:
            state['ammo'][ak] += mag * qty
    except:
        pass
