...
def handle_buy_armory(stdscr):
    curses.echo(); y0=12; stdscr.addstr(y0,2,'Armory: Choose #')
    idx=1
    for wp,inf in WEAPONS.items(): stdscr.addstr(y0+idx,2,f"{idx}. {wp} {inf['price']}cr"); idx+=1
    for u,pr in USEABLES.items(): stdscr.addstr(y0+idx,2,f"{idx}. {u} {pr}cr"); idx+=1
    for a,pr in AMMO_PRICES.items(): stdscr.addstr(y0+idx,2,f"{idx}. {a} {pr}cr"); idx+=1
    stdscr.addstr(y0+idx,2,'Q) Quit'); sel=stdscr.getstr().decode().strip(); curses.noecho()
    if sel.upper()=='Q': return
    try: choice=int(sel)
    except: return
    if choice<=len(WEAPONS): key=list(WEAPONS)[choice-1]; inf=WEAPONS[key]; state['credits']-=info['price']; state['weapons'][key]=state['weapons'].get(key,0)+1; state['ammo'][info['ammo']]+=info['mag']
    elif choice<=len(WEAPONS)+len(USEABLES): u=list(USEABLES)[choice-len(WEAPONS)-1]; state['credits']-=USEABLES[u]; state['useables'][u]+=1
    else: a=list(AMMO_PRICES)[choice-len(WEAPONS)-len(USEABLES)-1]; state['credits']-=AMMO_PRICES[a]; state['ammo'][a]+=5

# === Equip Useable !==
def handle_equip(stdscr):
    curses.echo(); stdscr.addstr(12,2,'Equip which? '); key=stdscr.getstr().decode().strip()
    curses.noecho()
    if key=='Health Pack' and state['useables'][key]>0: state['useables'][key]-=1; state['life']=clamp(state['life']+15,0,100)
    elif key=='Armor Kit' and state['useables'][key]>0 and state['armor']==0: state['useables'][key]-=1; state['armor']=20
...
def free_civilian(stdscr):
    ...
    curses.echo(); stdscr.addstr(5,2,'Pick a number 1-10: ')
    try: guess=int(stdscr.getstr().decode())
    except: return
    curses.noecho()
    if abs(guess-secret)<=2:
        state['people_freed']+=1
        stdscr.addstr(7,2,'Red pill! Mind freed.')
    else:
        stdscr.addstr(7,2,'Blue pill! Becomes Agent.')
        stdscr.refresh(); curses.napms(500)
        battle_screen(stdscr, smith=True)
    stdscr.addstr(9,2,'Press any key...'); stdscr.refresh(); stdscr.getkey()
